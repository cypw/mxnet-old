...
struct ImageRecParserParam : public dmlc::Parameter<ImageRecParserParam> {
  ...
  /*! \brief input shape */
  TShape data_shape;
  /*! \brief normalize input color channel */
  bool force2color;
  ...
  DMLC_DECLARE_PARAMETER(ImageRecParserParam) {
    ...
    DMLC_DECLARE_FIELD(force2color).set_default(false)
        .describe("Dataset Param: If set, always convert image to the color one, defalt: false.");
...
inline void ImageRecordIOParser<DType>::Init(
  ...
  if (param_.shuffle_chunk_size > 0) {
    ...
    if (num_shuffle_parts > 1) {
      LOG(INFO) << "InputSplitShuffle [" << param_.part_index << "/" << param_.num_parts << "]" 
                << ", seed(" << param_.shuffle_chunk_seed << ")";
...
ParseNext(std::vector<InstVector<DType>> *out_vec) {
  ...
  #pragma omp parallel num_threads(param_.preprocess_threads)
  {
    ...
    while (reader.NextRecord(&blob)) {
      ...
      if (param_.force2color) {
        // always convert image to the color one
        res = cv::imdecode(buf, CV_LOAD_IMAGE_COLOR);
      } else {
...
.add_arguments(ListTorchAugParams())
